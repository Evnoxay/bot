...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('unlock')
    .setDescription('Déverrouiller ton salon vocal'),
  execute: async (interaction) => {
    ...
    if (!data.isLocked) {
      const embed = new EmbedBuilder()
        ...
        .setDescription('Le salon est déjà déverrouillé!');
      return interaction.reply({ embeds: [embed], flags: 64 });
    }
    ...
    try {
      ...
      permissionManager.unlockChannel(channel.id);
      const embed = new EmbedBuilder()
        .setColor('#00FF00')
        .setTitle('🔓 Succès')
        .setDescription('Le salon est maintenant déverrouillé!');
